...
async def admin_manage_users(callback: types.CallbackQuery):
    ...
    try:
        ...
        if not users:
            ...
            return
        for u in users:
            builder.button(text=f"👤 {u['full_name']}", callback_data=f"admin_user_profile:{u['telegram_id']}")
...
async def admin_user_profile(callback: types.CallbackQuery, state: FSMContext):
    ...
    try:
        ...
        text = (
            ...
            f"🎁 **Yuborish balansi:** {user['give_balance']} coin\n💰 **Yiqqan balansi:** {user['earned_balance']} coin"
        )
        builder = InlineKeyboardBuilder()
...
async def admin_coin_add_save(message: types.Message, state: FSMContext):
    ...
    await message.answer(f"✅ *{user['full_name']}*ga {amount} coin qo'shildi!", parse_mode="Markdown", reply_markup=builder.as_markup())
...
async def admin_coin_sub_save(message: types.Message, state: FSMContext):
    ...
    builder = InlineKeyboardBuilder()
    builder.button(text="⬅️ Xodim profiliga qaytish", callback_data=f"admin_user_profile:{user_id}")
    await message.answer(f"✅ *{user['full_name']}*ning balansidan {amount} coin chegirildi!", parse_mode="Markdown", reply_markup=builder.as_markup())
...
async def admin_user_ban(callback: types.CallbackQuery):
    ...
    try:
        ...
        if user:
            await reject_user_db(user_id)
            await callback.message.edit_text(f"❌ *{user['full_name']}* muvaffaqiyatli tizimdan o'chirildi.", parse_mode="Markdown", reply_markup=get_admin_menu())
...
async def show_top_5(callback: types.CallbackQuery):
    ...
    try:
        ...
        if not top_list: text += "_Hali bo'sh_"
            ...
            for i, row in enumerate(top_list): text += f"{i+1}. *{row['full_name']}* — `{row['earned_balance']} coin`\n"
...
async def show_history(callback: types.CallbackQuery):
    ...
    try:
        ...
        if not rows:
            ...
            for r in rows:
                if callback.from_user.id == ADMIN_ID:
                    text += f"👤 *{r['sender']}* ➡️ *{r['receiver']}*\n💰 {r['amount']} coin | 🎯 Sabab: _{r['reason']}_\n📅 _{r['timestamp']}_\n---------------------\n"
                else:
                    if r['sender_id'] == callback.from_user.id:
                        text += f"📤 *Yuborildi:* {r['amount']} coin ➡️ *{r['receiver']}*\n"
                    else:
                        text += f"📥 *Olingan:* {r['amount']} coin ⬅️ *{r['sender']}*\n"
                    text += f"🎯 *Sabab:* _{r['reason']}_\n📅 _{r['timestamp']}_\n---------------------\n"
...
async def daily_spin(callback: types.CallbackQuery):
    ...
    try:
        ...
        if user and user['last_spin_date'] == today_str:
...
async def start_transfer_inline(callback: types.CallbackQuery, state: FSMContext):
    ...
    try:
        ...
        builder = InlineKeyboardBuilder()
        if not users:
            await callback.message.edit_text("👤 **Botda hozircha boshqa tasdiqlangan xodimlar majvud emas.**", reply_markup=get_back_button())
            return
        for u in users:
            builder.button(text=f"👤 {u['full_name']}", callback_data=f"select_user:{u['telegram_id']}")
        builder.button(text="⬅️ Bosh Menyu", callback_data="menu_main")
        builder.adjust(1)
...
async def process_selected_user(callback: types.CallbackQuery, state: FSMContext):
    ...
    try:
        ...
        text = (
            f"👤 **Qabul qiluvchi:** *{receiver['full_name']}*\n\n"
